...
export default function InventoryRequestsPanel({ role, onToast }: Props) {
  ...
  useEffect(() => { void load(); return () => streamRef.current?.getTracks().forEach((track) => track.stop()) }, [])
  ...
  async function openCamera() {
    ...
    try {
      ...
      const scan = async () => {
        ...
        if (codes[0]?.rawValue) {
          setForm((current) => ({ ...current, scanned_code: codes[0].rawValue }))
          setCameraOpen(false)
          streamRef.current?.getTracks().forEach((track) => track.stop())
          onToast('Box code scanned')
        } else window.setTimeout(() => void scan(), 500)
      }
      window.setTimeout(() => void scan(), 700)
  ...
  return <>
    {role === 'warehouse_worker' && <section className="panel workflow-card">
      ...
      <form className="stack-form" onSubmit={submit}>
        ...
        <button className="button subtle" type="button" onClick={() => void openCamera()}>Camera scan</button>
        {cameraOpen && <video ref={videoRef} autoPlay playsInline className="camera-preview" />}
        {cameraOpen && <button className="button subtle" type="button" onClick={() => { setCameraOpen(false); streamRef.current?.getTracks().forEach((track) => track.stop()) }}>Stop camera</button>}
